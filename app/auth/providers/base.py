...
3. `providers/__init__.py`(또는 레지스트리)에 인스턴스 등록

`SocialProfile` 은 provider 별 응답을 서버 공통 표현으로 정규화한 값이다.
`EmailConsentRequired` 는 이메일 미동의(제공 거부) 상황 — 라우터가 가입 거부 + 연결 해제로 처리한다.
"""
...
class EmailConsentRequired(SocialAuthError):
    """유저가 이메일 제공에 동의하지 않은 경우.

    provider access token 을 함께 전달해 라우터가 연결 해제(unlink)를 호출할 수 있게 한다.
    """

    def __init__(self, access_token: str) -> None:
        super().__init__("이메일 제공 동의가 필요합니다.")
...
class SocialProfile:
    ...
    provider: str
    provider_id: str  # provider 고유 유저 id (문자열)
    email: str
...
class SocialProvider(ABC):
    ...
    async def fetch_profile(
        ...
        """access token 으로 유저 프로필 조회 → 정규화. 이메일 미동의 시 EmailConsentRequired."""

    async def unlink(self, access_token: str, client: httpx.AsyncClient) -> None:
        """(옵션) 이메일 미동의 등으로 가입을 거부할 때 연결을 해제한다. 기본은 no-op."""
        return None
