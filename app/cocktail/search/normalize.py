...
# 검색어 길이 상한. 카탈로그(622건)의 최장 이름은 한글 18자 / 영문 35자이므로
# 40이면 모든 이름을 끝까지 담을 수 있어 어떤 항목이든 exact 매칭이 보존된다.
# 그러면서도 상한 자체는 유지한다: 한글은 자모 분해로 길이가 ~2.4배가 되고 요청당
# 전수 스캔 비용이 입력 길이에 비례하므로, 상한이 최악 케이스 CPU를 직접 제한한다.
MAX_LEN = 40
