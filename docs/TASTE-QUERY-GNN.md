...
공개 엔드포인트의 비용을 제한하기 위해 실제 클라이언트 IP당 분당 20회로 제한한다. 한 개 이상의 맛을 선택한 요청은 descriptor ID 순서와 무관하게 같은 조합으로 정규화해 프로세스 내부 LRU 캐시에 최대 4,096개까지 24시간 보관한다. 빈 배열의 랜덤 추천은 캐시를 우회하므로 호출할 때마다 새로 선택한다.
